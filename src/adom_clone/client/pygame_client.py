...
def run_game() -> None:
    pygame.init()
    pygame.display.set_caption("ADOM Clone - Phase 2")
...
def _draw(screen: pygame.Surface, font: pygame.font.Font, session: GameSession) -> None:
    ...
    map_line = f"Map: {tile_map.kind.value} | HP: {session.player_hp_text}"
    controls_line = "Move: WASD/Arrows | G: pickup | 1-9: use slot | R: drop last | .: wait"
    inventory_line = _inventory_line(session)
    lines = [map_line, controls_line, inventory_line, *session.messages[-2:]]
